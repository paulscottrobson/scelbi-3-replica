...
static int addressSettings[] = { 0,0,0,0xFFFF }; 									// Adjustable values : Code, Data, Other, Break.
...
void GFXXRender(SDL_Surface *surface) {
	...
	if (currentKey != lastKey) {													// Key changed
		...
		currentKey = DEBUG_KEYMAP(currentKey,inRunMode != 0);						// Pass keypress for processing.
